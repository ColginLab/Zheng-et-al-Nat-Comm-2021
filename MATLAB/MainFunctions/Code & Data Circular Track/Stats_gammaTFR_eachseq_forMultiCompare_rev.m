...
% fig_out = [parentfd,'\Figure_Pxn_gammaTFR_sequence_reverse'];
%fig_out = [parentfd,'\GroupData Figures'];
fig_out = 'E:\ColginLab\Figures\SupFigure4';
if ~isdir(fig_out)
    mkdir(fig_out)
end
file_input1 = 'group_gammaTFR_eachseq_20190723.mat'; % 4 rats % remove jumping-out points, ind_approach = 3;
...
title12{2} = 'Err samp trials';

ind_seq{3} = find(data_info(:,3) == 3 & ...
    data_info(:,4) <= 8 & ...
    pos_real(:,2) < data_info(:,8) &... % stop location
    data_info(:,10) >= ncell & data_info(:,11) >= nspk & ...  % limit active cells number
    data_info(:,6) == 1 &...  % only correct or error trials
    ...
    para_all(:,1) <= 20 & para_all(:,4) >= nmove &...
    dist_prdc_real <= 5*bin_ang);
title12{3} = 'Crt test trials';

ind_seq{4} = find(data_info(:,3) == 3 & ...
    data_info(:,4) <= 8 & ...
    pos_real(:,2) < data_info(:,8) &... % stop location
    data_info(:,10) >= ncell & data_info(:,11) >= nspk & ...  % limit active cells number
    data_info(:,6) == 0 &...  % only correct or error trials
...
%         predloc0_max(ii,itrial) = max(Pxn0{ii,4});
        predloc0_max(ii,itrial) = ang_bins(max(Pxn0{ii,end}));
    end
end
...
if strcmp(ind_Type{i1},'all')
    StatsData = array2table(Z,'VariableNames',{'TrialType','CrtErr','PosBin','Slope'});
...
for ff = 1:length(figh)
    if strcmp(ind_Type{i1},'all')
        ...
        for i = 1:length(measure_x)
            h1 = errorbar(dist_bins_center(1:6),measure_x{i}(1,1:6),measure_x{i}(2,1:6),'s-','LineWidth',1,'Color',color{i});
            %errorbar_tick(h1, 0);
        end
...
for itrial = 1:Ntrial
    ...
    for ii = 1:n_seq0(itrial)
        ...
        predloc_shift1{ii,itrial} = Pxn0{ii,9}+n_shift;
        ...
        predloc_shift2{ii,itrial} = Pxn0{ii,9}+n_shift;
    end
end


% Plot all prediction lines for all sequences
sign_plot = 1;
locNumb = -12:1;
if sign_plot && strcmp(ind_Type{i1},'ds')
    ...
    for iplot = 1:Ntrial
        ...
        for ii = 1:length(pxn0_shift)
            ...
            if predloc(2)-predloc(1) > pi
                predloc(2) = predloc(2)-2*pi;
            end
...
saveas(ffa1,[fig_out,'\SeqSlopeNeg_average_samp'],'epsc')
saveas(ffa1,[fig_out,'\SeqSlopeNeg_average_samp'],'fig')
saveas(ffa2,[fig_out,'\SeqSlopeNeg_allSeq_test'],'epsc')
saveas(ffa2,[fig_out,'\SeqSlopeNeg_allSeq_test'],'fig')
close all

%% Save stats output
save([fig_out,'\Stats_SeqSlopeNeg.mat'],'StatsOut','StatsData')