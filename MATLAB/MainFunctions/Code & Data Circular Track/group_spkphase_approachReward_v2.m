...
function group_spkphase_approachReward_v2(DownSample)
...
%file_output = [parentfd,'\GroupData Figures\group_spkphase_v2',downsample_str,'.mat'];
file_output = ['E:\ColginLab\Data Analysis\GroupData\group_spkphase_v2',downsample_str,'.mat'];
...
for ns = 1:isession
    ...
    if SpkInSequnece
        ...
        t_startstop = [cellfun(@min,seq.Pxn_all(in & ind_seq_log,1)),... 
    ...
    for ii = 1:numGroup
        ...
        if ~isempty(spk2phase_ind)
            for cc = 1:length(ConditionType)
                ...
                in_sequence = ismember(spk2phase_ind,InSequence);
...
h1 = figure('Units','normalized','Position',[0 0 1 1/3]);
for cc = 1:length(ConditionType)
    
    subplot(1,length(ConditionType),cc)
    imagesc(1:numGroup,PhaseAxis(1:end-1),squeeze(SpkPhasePro_theta_norm(1:end-1,:,cc)))
    if cc == 1
        ylabel(sprintf('Theta spike\nphase (degrees)'))
    end
    axis xy square
    set(gca,'XTick',1:3,'XTickLabel',{'Early','Middle','Late'})
    xlabel('Place field location')
    set(gca,'CLim',[0 .05])
    colorbar
    title(ConditionType{cc})
end
