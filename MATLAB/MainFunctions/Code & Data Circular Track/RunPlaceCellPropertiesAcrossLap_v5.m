function RunPlaceCellPropertiesAcrossLap_v5(Runfd,trackdata,Outdir,cellList,spktime_fd)
% This version set speed threshold to 0 cm/s
Condition = {'Prerunning';'Sample';'Test';'Postrunning'};
outdir = strcat(Outdir,'\LapFRcorr_noSpeedThr');
if ~isdir(outdir)
    mkdir(outdir)
end
...
for rr = 1:size(data_angle,2)
    ...
    for ll = 1:lapN
        ...
        occupancy = Kerneldensity_circular(lap_pos(run_ind),posbinaxis/180*pi,4/180*pi);
        occupancy = occupancy*sum(run_ind); % in units of sampling points
        ...
        for ii = 1:length(cellID)
            ...
            if sum(run_ind_eeg & spkind_log) > 5
                spkposcount = Kerneldensity_circular(lap_pos_eeg(run_ind_eeg & spkind_log),posbinaxis/180*pi,4/180*pi);
                spkposcount = spkposcount*sum(run_ind_eeg & spkind_log);
                LapSpkind.(Condition{rr}){ii,ll} = lap_pos_eeg(run_ind_eeg & spkind_log);
            else
                spkposcount = zeros(size(occupancy));
            end
            LapFR.(Condition{rr})(ii,:,ll) = spkposcount./occupancy*sampfreq;            
        end
...
function [occupancy] = Kerneldensity_circular(phase,pbins,width)
kappa = 2*pi/width; % larger values correspond to concentrated distribution
delta = angle(exp(1i*repmat(phase,1,length(pbins))).*conj(exp(1i*repmat(pbins,length(phase),1))));
W = exp(kappa*cos(delta)); % von Mises distribution
D = ones(1,length(phase))*exp(kappa*cos(delta));
occupancy = sum(W)./sum(D);