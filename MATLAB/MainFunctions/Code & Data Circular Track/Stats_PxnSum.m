...
function Stats_PxnSum(AnalysisDir, FiguresDir)

% Load data
%%parentfd = fileparts(mfilename('fullpath'));
% load([parentfd,'\GroupData\group_PxnSum_5LocAway_25cells_20190518_5bins_v3_ds.mat'])
% load([parentfd,'\GroupData\group_Pxn_forErr_5LocAway_25cells_20190518_5bins_v3_ds.mat'])
%%load([parentfd,'\GroupData\group_PxnSum_5LocAway_0cells_20190518_5bins_v3_ds.mat'])
%%load([parentfd,'\GroupData\group_Pxn_forErr_5LocAway_0cells_20190518_5bins_v3_ds.mat'])
load(strcat(AnalysisDir,'group_PxnSum_5LocAway_0cells_20190518_5bins_v3_ds.mat'))
load(strcat(AnalysisDir,'group_Pxn_forErr_5LocAway_0cells_20190518_5bins_v3_ds.mat'))
...
dirTypes = {'ahead','behind'};
for dirInd = 1:length(dirTypes)
    dirType = dirTypes{dirInd};
    if strcmp(dirType,'ahead')
        i_samp = 6; i_test = 12; % ahead probability
    elseif strcmp(dirType,'behind')
        i_samp = 7; i_test = 13; % behind probability
    end
%%  Group data: 
...
ffa1 = figure('Units','normalized','Position',[0 0 0.4 0.6]);
...
for ns = 1:isession
    ...
    if ~isempty(ind)
        ...
        if ~isempty(ind_correct_early) && ~isempty(ind_correct_late)
            ...
            pxn_samp1_crt(ns,3) = group_pxn(ind_correct_early(end),i_test); % last correct test
            pxn_samp1_crt(ns,4) = group_pxn(ind_correct_late(end),i_test); % last correct test
...
sample_color = [255,165,0]./255;
test_color = [111,57,214]./255;
ffa3 = figure('Units','normalized','Position',[0 0 0.4 0.6]);
data0_mean = mean(data0);
data0_sem = std(data0)./sqrt(size(data0,1));
hold on
...
h4 = errorbar(3,data0_mean(4),data0_sem(4),'color','k','LineWidth',2);
%%errorbar_tick(h1, 0);
%%errorbar_tick(h2, 0);
%%errorbar_tick(h4, 0);
% plot([1,2,3],data0,'ko');
...
ylim(ytick0([1,end]))
set(gca,'YTick',ytick0);
set(gca,'XTickLabel',{'Sample1','E-crt test1','L-crt testN'});
set(gca,'fontsize',20);
if strcmp(dirType,'ahead') % ahead probability
    ylabel([{'Summation of P(x|n)'},{'from current location to reward location'}]);
elseif strcmp(dirType,'behind') % behind probability
...
pxn_all = [pxn_samp1_crt,pxn_err_mins_plus];
angvel_all = [angvel_samp1_crt,angvel_err_mins_plus];
...
ffa4 = figure('Units','normalized','Position',[0 0 0.4 0.6]);
data0_mean = mean(data0);
data0_sem = std(data0)./sqrt(size(data0,1));
hold on
bar(1,data0_mean(1),'FaceColor',sample_color,'EdgeColor', sample_color);
...
test_color = [111,57,214]./255;
test_err_color = [120,120,120]./255;
ffa5 = figure('Units','normalized','Position',[0 0 0.4 0.6]);
hold on
...
saveas(ffa2,[FiguresDir,'\PxnSum_line_',dirType],'epsc')
saveas(ffa2,[FiguresDir,'\PxnSum_line_',dirType],'fig')
% saveas(ffa3,[fig_out,'\PxnSum_bar_',dirType],'epsc')
% saveas(ffa3,[fig_out,'\PxnSum_bar_',dirType],'fig')
close all
...
save([FiguresDir,'\Stats_PxnSum.mat'],'StatsOut','StatsData_LineGraph','within_tbl2')